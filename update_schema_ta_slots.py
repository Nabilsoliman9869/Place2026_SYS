...
def update_schema():
    try:
        ...
        cursor = conn.cursor()
        print(">>> Adding TA Slots Extra Fields...")

        # TASchedules Table
        fields = [
            ("InterviewType", "NVARCHAR(50)") # Zoom, Phone, Onsite
        ]
        
        for col, dtype in fields:
            cursor.execute(f"""
                IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='TASchedules' AND COLUMN_NAME='{col}')
                BEGIN
                    ALTER TABLE TASchedules ADD {col} {dtype};
                    PRINT 'Added {col} to TASchedules';
                END
            """)

...
<html lang="ar" dir="rtl">
  ...
  <body class="bg-dark d-flex align-items-center justify-content-center" style="min-height: 100vh;">
    <div class="card shadow p-4" style="width: 500px;">
      ...
      <h5 class="text-center text-light">أدوات قاعدة البيانات</h5>
      <form method="post" action="{{ url_for('init_db_route') }}" onsubmit="return confirm('هل أنت متأكد؟ سيتم إنشاء الجداول إذا لم تكن موجودة.');">
          <div class="d-grid gap-2">
      ...
      <div class="text-center mt-3">
        <a href="{{ url_for('login') }}" class="btn btn-link text-light">عودة لتسجيل الدخول</a>
      </div>
    </div>
  </body>
</html>
...
def query_db(query, args=(), one=False):
    ...
    try:
        ...
        # print(f"Query Error: {e}") # Log error but don't crash app if possible
        cursor.close()
        raise e
...
def init_db_from_schema():
    ...
    try:
        ...
        # Split script by GO commands (common in MSSQL scripts)
        ...
        for command in commands:
            if command.strip():
                try:
                    ...
                    # Continue or fail? Let's continue and report errors

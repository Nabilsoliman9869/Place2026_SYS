...
def update_schema():
    try:
        ...
        cursor = conn.cursor()
        print(">>> Adding Extra Marketing Fields...")

        # Candidates Table
        fields = [
            ("MarketingAssessment", "NVARCHAR(MAX)"), # Free text for initial assessment
            ("PreviousApplicationDate", "DATETIME")   # For 3-month rule check
        ]

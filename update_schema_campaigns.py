...
def update_schema():
    try:
        ...
        cursor = conn.cursor()
        print(">>> Adding Missing Campaign Fields...")
        ...
        for col, dtype in fields:
            cursor.execute(f"""
                IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='Campaigns' AND COLUMN_NAME='{col}')
                BEGIN
                    ALTER TABLE Campaigns ADD {col} {dtype};
                    PRINT 'Added {col} to Campaigns';
                END
            """)
